...
subplot(2,3,5); axis([0 360 -20 20]); grid on; title('Angular velocity of the rod'); xlabel('Angle of the rod'); ylabel('Angular Velocity of the rod')
linewx=animatedline('Color','b');linewy=animatedline('Color','r');linewz=animatedline('Color','k');
linewxpoint=animatedline('Color','b','Marker','o','MarkerSize',10,'MarkerFaceColor','b','HandleVisibility','off');linewypoint=animatedline('Color','r','Marker','o','MarkerSize',10,'MarkerFaceColor','r','HandleVisibility','off');linewzpoint=animatedline('Color','k','Marker','o','MarkerSize',10,'MarkerFaceColor','k','HandleVisibility','off');
subplot(2,3,6); axis([0 360 -20 20]); grid on; title('Angular acceleration of the rod'); xlabel('Angle of the rod'); ylabel('Angular Velocity of the rod')
lineax=animatedline('Color','b');lineay=animatedline('Color','r');lineaz=animatedline('Color','k');
lineaxpoint=animatedline('Color','b','Marker','o','MarkerSize',10,'MarkerFaceColor','b','HandleVisibility','off');lineaypoint=animatedline('Color','r','Marker','o','MarkerSize',10,'MarkerFaceColor','r','HandleVisibility','off');lineazpoint=animatedline('Color','k','Marker','o','MarkerSize',10,'MarkerFaceColor','k','HandleVisibility','off');
%% Buttons %%
...
while true
    ...
    subplot(2,3,5)
    clearpoints(linewxpoint);clearpoints(linewypoint);clearpoints(linewzpoint);
    if q==1
        addpoints(linewx,theta,wx);addpoints(linewy,theta,wy);addpoints(linewz,theta,wz);
    end
    addpoints(linewxpoint,theta,wx);addpoints(linewypoint,theta,wy);addpoints(linewzpoint,theta,wz);
    subplot(2,3,6)
    if i>1
        ax=wx-pwx;ay=wy-pwy;az=wz-pwz;
        clearpoints(lineaxpoint);clearpoints(lineaypoint);clearpoints(lineazpoint);
        if q==1
            addpoints(lineax,theta,ax);addpoints(lineay,theta,ay);addpoints(lineaz,theta,az);
        end
        addpoints(lineaxpoint,theta,ax);addpoints(lineaypoint,theta,ay);addpoints(lineazpoint,theta,az);    
    end
    %% Time Control %%
    to=toc;
    if to < (1/framerate)
