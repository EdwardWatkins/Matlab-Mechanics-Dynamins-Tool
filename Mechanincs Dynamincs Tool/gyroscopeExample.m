...
precession = 30; % Period of one precession rotation
nutationMidangle = 60; % Starting nutation angle
nutationMaxvariation = 10; % Variance of the nutation angle
nutationPeriod = 15; % Period of a sin wave nutation cycle
spin = 10; % Period of a spin rotation
...
Matrix = inertiaSetup(shape,x,y,z);
%% Position Components %%
...
while t <= Tmax
    ...
    %% Inertia %%
    rI(i,:) = rotatedInertia(Matrix,0,0,0,[0,0,0],[0,0,0],spinningTopMass);
    rIXYZ(i,:) = rotatedInertia(Matrix,0,0,0,[0,0,0],[X,Y,Z],spinningTopMass);
