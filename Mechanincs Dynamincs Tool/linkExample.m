...
R=sqrt((xm-Ox)^2);
[rmin,rmax,alphaMax,alphaMin,betaMax,betaMin,startAlpha,startBeta,perimiter]=linkInfo(v1,p1,v2,p2,L);
%% Make components %%
...
subplot(2,2,2); axis([alphaMin*1.2 alphaMax*1.2 betaMin*1.2 betaMax*1.2]); grid on; title('Link position'); xlabel('Position along shaft 1'); ylabel('Position along shaft 2')
...
subplot(2,2,3); axis([0 360 -inf inf]); grid on; title('Relative velocity of link ends'); xlabel('Angle of the rod'); ylabel('Distance traveled along the shaft per degree of motion')
...
subplot(2,2,4); axis([0 360 -inf inf]); grid on; title('Relative acceleration of link ends'); xlabel('Angle of the rod'); ylabel('Acceleration of link motion per degree of motion')
...
%% Clickable Legend %%
subplot(2,2,2); legend1 = legend(); legend1.String='Position';legend1.ItemHitFcn = @clickableLegend;
subplot(2,2,3); legend2 = legend('Link 1','Link 2'); legend2.ItemHitFcn = @clickableLegend;
subplot(2,2,4); legend3 = legend('Link 1','Link 2'); legend3.ItemHitFcn = @clickableLegend;
%% Loop %%
theta=0; i = 1; q = 1;
...
while true
    ...
    l2=patch('Faces',link1f,'Vertices',[link2xxxnew,link2yyynew,link2zzznew],'EdgeColor','none','FaceColor','green');
    r=patch('Faces',rodf,'Vertices',[rodxxxtemp,rodyyytemp,rodzzztemp],'EdgeColor','none','FaceColor','yellow');
    subplot(2,2,2); clearpoints(aniline1pospoint);
    ...
    if i > 1
        subplot(2,2,3); clearpoints(aniline1velpoint); clearpoints(aniline2velpoint);
        ...
        if i > 2
            subplot(2,2,4); clearpoints(aniline1accpoint); clearpoints(aniline2accpoint);
    ...
    if q == 1
        if i > 1
            subplot(2,2,3);addpoints(aniline1vel,theta,(link1delta/(v1/norm(v1))));addpoints(aniline2vel,theta,(link2delta/(v2/norm(v2))));
            if i > 2
                subplot(2,2,4);addpoints(aniline1acc,theta,link1acc);addpoints(aniline2acc,theta,link2acc);
            end
        end
    end
    drawnow
    if q == 0
        ...
        subplot(2,2,3);axis([0 360 min(velocity(:)) max(velocity(:))])
        subplot(2,2,4);axis([0 360 min(acceleration(:)) max(acceleration(:))])
    elseif i > 3
