...
bigDiskSpeed = rad2deg(3); % Big Disk angular rotation in degrees per second
Tmax = 360/bigDiskSpeed; % Animate for 1 rotation
pointX = 1; pointY = 0; pointZ = 0.25; % Starting co-ordinates of the point to measure
smallDiskRadius = 0.25; % Dimension of the small disk
smallDiskMountPointRadius = 1; % Used to link the speed of the two disks
smallDiskSpeed = (smallDiskMountPointRadius/smallDiskRadius)*bigDiskSpeed; % Small Disk angular rotation in degrees per second
smallDiskMass = 2.5; % Mass of the small disk. Used when calculating inertia
animationSpeed = 0.25; % Controls the speed of animation. Useful with high or low speeds
framerateLimit = 12; % FPS - Lower this number if the graphs have added noise
...
smallDisk = vectorCylinder(smallDiskRadius,[0.95,0,0],[1.05,0,0],x,y,z);
...
[X1,Y1,Z1] = massCenter(bigDisk,x,y,z);
[X2,Y2,Z2] = massCenter(smallDisk,x,y,z);
Matrix = inertiaSetup(smallDisk,x,y,z);
...
rI = zeros(maxFrames,3); rIXYZ = zeros(maxFrames,3);
%% Loop %%%
while t <= Tmax
    tic;
    %% Plotting %%
    ...
    rI(i,:) = rotatedInertia(Matrix,0,0,currentAngle,[0,0,0],[0,0,0],smallDiskMass);
    rIXYZ(i,:) = rotatedInertia(Matrix,0,0,currentAngle,[0,0,0],[X2,Y2,Z2],smallDiskMass);
